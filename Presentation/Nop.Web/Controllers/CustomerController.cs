...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BaseNopController
    {
        ...
        private readonly ISharingPartnerLevelService _sharingPartnerLevelService;
        ...
        public CustomerController(IAuthenticationService authenticationService,
            ...
            CaptchaSettings captchaSettings, ExternalAuthenticationSettings externalAuthenticationSettings,ISharingPartnerLevelService sharingPartnerLevelService)
        {
            ...
            this._sharingPartnerLevelService = sharingPartnerLevelService;
        ...
        protected void PrepareCustomerInfoModel(CustomerInfoModel model, Customer customer, bool excludeProperties)
        {
            ...
            model.PartnerID = (customer.PartnerID.HasValue ? customer.PartnerID.Value : 0).ToString();
            model.SharingPartnerLevel = (customer.SharingPartnerLevel.HasValue ? customer.SharingPartnerLevel.Value : 0);
            model.AvailableSharingPartnerLevels.Add(new SelectListItem() { Text = "Sharing Partner", Value = "0" });
            foreach (var s in _sharingPartnerLevelService.GetAllSharingPartnerLevel())
            {
                model.AvailableSharingPartnerLevels.Add(new SelectListItem()
                {
                    Text = s.GetLocalized(x => x.SharingPartnerLevelName),
                    Value = s.Id.ToString(),
                    Selected = (s.Id == (customer.SharingPartnerLevel.HasValue ? customer.SharingPartnerLevel.Value : 0))
                });
            }
            model.Skype = customer.Skype;
            model.Facebook = customer.Facebook;
            model.Twitwer = customer.Twitwer;
            if (!excludeProperties)
            {
                ...
                //model.PartnerID = (customer.PartnerID.HasValue ? customer.PartnerID.Value : 0).ToString();
                //model.SharingPartnerLevel = (customer.SharingPartnerLevel.HasValue ? customer.SharingPartnerLevel.Value : 0);
                //model.AvailableSharingPartnerLevels.Add(new SelectListItem() { Text = "Selected SharingPartner", Value = "0" });
                //foreach (var s in _sharingPartnerLevelService.GetAllSharingPartnerLevel())
                //{
                //    model.AvailableSharingPartnerLevels.Add(new SelectListItem()
                //    {
        ...
        public ActionResult Register()
        {
            ...
            foreach (var s in _sharingPartnerLevelService.GetAllSharingPartnerLevel())
            {
                model.AvailableSharingPartnerLevels.Add(new SelectListItem()
                {
                    Text = s.GetLocalized(x => x.SharingPartnerLevelName),
                    Value = s.Id.ToString(),
                    Selected = (s.Id == model.SharingPartnerLevel)
                });
            }
            if (_customerSettings.CountryEnabled)
            {
                ...
                if (_customerSettings.StateProvinceEnabled)
                {
                    ...
                    {
                        var states1 = _stateProvinceService.GetStateProvincesByCountryId(1).ToList();
        ...
        public ActionResult Register(RegisterModel model, string returnUrl, bool captchaValid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    //Add Field Customer
                    int outid = 0;
                    int.TryParse(model.TaxID, out outid);
                    customer.TaxID = outid;
                    int.TryParse(model.PartnerID, out outid);
                    customer.PartnerID = outid;
                    customer.SharingPartnerLevel = model.SharingPartnerLevel;
                    customer.Facebook = model.Facebook;
                    customer.FanPage = model.FanPage;
                    customer.Skype = model.Skype;
                    customer.Twitwer = model.Twitwer;
                    _customerService.UpdateCustomer(customer);
        ...
        public ActionResult Info(CustomerInfoModel model)
        {
            ...
            try
            {
                ...
                {
                  //  int partnerID = 0;
                  //  int.TryParse(model.PartnerID, out partnerID);
                  ...
                  //  customer.SharingPartnerLevel = model.SharingPartnerLevel;
                    ...
                    //form fields
                    //_genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.PartnerID, model.PartnerID);

                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.FanPage, model.FanPage);
                   // _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.SharingPartnerLevel, model.SharingPartnerLevel);
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.Facebook, model.Facebook);
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.Skype, model.Skype);
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.TaxID, model.TaxID);

                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.Twitwer, model.Twitwer);
                    if (_customerSettings.GenderEnabled)
                        _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.Gender, model.Gender);
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.FirstName, model.FirstName);
                    _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.LastName, model.LastName);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult BeYoungTOTALHEALTH(CustomerInfoModel model)
        {
            if (!IsCurrentUserRegistered())
                return new HttpUnauthorizedResult();
            var customer = _workContext.CurrentCustomer;
        ...
        public ActionResult BeYoungTOTALHEALTH()
        {
            if (!IsCurrentUserRegistered())
                return new HttpUnauthorizedResult();
            var customer = _workContext.CurrentCustomer;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Social(CustomerInfoModel model)
        {
            if (!IsCurrentUserRegistered())
                return new HttpUnauthorizedResult();
            var customer = _workContext.CurrentCustomer;
