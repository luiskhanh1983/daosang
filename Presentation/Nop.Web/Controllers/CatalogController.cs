...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BaseNopController
    {
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                ...
                if (preparePictureModel)
                {
                    ...
                    {
                        var picture = _pictureService.GetPicturesByProductId(product.Id, 2).ToList();
                        if (picture.Count >= 2)
                        {
                            var pictureModel = new PictureModel()
                            {
                                ImageUrl = _pictureService.GetPictureUrl(picture[1], pictureSize),
                                FullSizeImageUrl = _pictureService.GetPictureUrl(picture[1]),
                                ...
                                SecondImageUrl = _pictureService.GetPictureUrl(picture[0], pictureSize),
                                SecondFullSizeImageUrl = _pictureService.GetPictureUrl(picture[0]),
                                SecondTitle = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat"), model.Name),
                                SecondAlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat"), model.Name),

                            };
                            return pictureModel;

                        }
                        else if (picture.Count == 0)
                        {
                            var pictureModel = new PictureModel();
                            model.DefaultPictureModel = _cacheManager.Get(defaultProductPictureCacheKey, () =>
                            {
                                var cachedPicture = _pictureService.GetPicturesByProductId(product.Id, 1).FirstOrDefault();
                                pictureModel.ImageUrl = _pictureService.GetPictureUrl(cachedPicture, pictureSize);
                                pictureModel.FullSizeImageUrl = _pictureService.GetPictureUrl(cachedPicture);
                                pictureModel.Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat"), model.Name);
                                pictureModel.AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat"), model.Name);
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                foreach (var picture in pictures)
                {
                    pictureModels.Add(new PictureModel()
                    {
                        ...
                        Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), model.Name),
                        AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name),
                        DefaultImageUrl = _pictureService.GetPictureUrl(picture, defaultPictureSize)
                    });
                }
        ...
        public ActionResult Category(int categoryId, CatalogPagingFilteringModel command)
        {
            ...
            if (category.Name == "live webinars")
            {
                if (_workContext.CurrentCustomer.IsRegistered())
                {
                    var livewebinarsModel = new CategoryModel.LivewebinarsModel();
                    var address = _workContext.CurrentCustomer.Addresses.FirstOrDefault();
                    livewebinarsModel.FirstName = address != null ? address.FirstName : "";
                    livewebinarsModel.PhoneNumber = address != null ? address.PhoneNumber : "";
                    livewebinarsModel.ContryCode = address != null ? address.Country.TwoLetterIsoCode : "";
                    livewebinarsModel.ContryName = address != null ? address.Country.Name : "";
                    livewebinarsModel.Email = _workContext.CurrentCustomer.Email;
                    model.Livewebinar = livewebinarsModel;
                    return View("Livewebinars", model);
                }
                else
                {
                    return RedirectToAction("Login", "Customer", new { returnUrl = this.Request.Url.AbsolutePath });

                }
            }
            if (category.Name == "SYSTEM STATS")
            {
                ...
                Nop.Web.Models.Catalog.CategoryModel.BybbstatsModel bybbstatsmodel = new CategoryModel.BybbstatsModel();
